...
namespace Framework.Components.ApplicationUser
{
    public partial class ApplicationDataManager : StandardDataManager
    {
        ...
        public static List<ApplicationDataModel> GetList(RequestProfile requestProfile)
        {
            return GetEntityDetails(ApplicationDataModel.Empty, requestProfile, 0);
        }

		public static List<ApplicationDataModel> GetApplicationList(RequestProfile requestProfile)
		{
			var list = GetEntityDetails(ApplicationDataModel.Empty, requestProfile, 0);

			var result = list.Select(item => new ApplicationDataModel()
			{
					Name = item.Name
				,	Code = item.Code
			...
			result.Insert(0, new ApplicationDataModel()
			{
					Name = "All"
				,	ApplicationId = -1
			});	
        ...
        public static string ToSQLParameter(ApplicationDataModel data, string dataColumnName)
        {
            ...
            switch (dataColumnName)
            {
				...
				case ApplicationDataModel.DataColumns.Code:
					if (!string.IsNullOrEmpty(data.Code))
					{
						returnValue = string.Format(SQL_TEMPLATE_PARAMETER_STRING_OR_DATE, ApplicationDataModel.DataColumns.Code, data.Code);
					}
					else
					{
						returnValue = string.Format(SQL_TEMPLATE_PARAMETER_NULL, ApplicationDataModel.DataColumns.Code);
					}
					break;
        ...
        private static string CreateOrUpdate(ApplicationDataModel data, RequestProfile requestProfile, string action)
        {
            ...
            sql = sql + ", " + ToSQLParameter(data, ApplicationDataModel.DataColumns.ApplicationId) +
						...
						", " + ToSQLParameter(data, ApplicationDataModel.DataColumns.Code);
            return sql;
        }
        ...
        public static bool DoesExist(ApplicationDataModel data, RequestProfile requestProfile)
        {
