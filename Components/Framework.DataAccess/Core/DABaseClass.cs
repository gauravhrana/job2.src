...
namespace Framework.PM.Common.DataAccess
{
	...
	public class DASQLBase : DataObject, IDisposable
	{
		...
		public void ExecuteNonQueryBatch(bool useTransaction, int commandTimeout)
		{
			...
			PrepareCommand(CommandType.Text, sm_BeginBatch + m_BatchText.ToString());

			int oldTimeout = m_Command.CommandTimeout;
			...
			try
			{
				...
				try
				{
					...
					if (m_Reader != null)
					{
						while (m_Reader.Read())
						{
							ServerResponse sr = new ServerResponse(m_Reader);
		...
		public void ExecuteNonQuery(CommandType commandType, string commandText, params IDataParameter[] commandParams)
		{
			...
			while (true)
			{
				...
				catch (SqlException sqlEx)
				{
					...
					attempt++;
					System.Threading.Thread.Sleep(sqlRetryTimeout);
				}
				catch (Exception ex)
				{
					LogSqlError(ex);
		...
		public object ExecuteReturnParam(CommandType commandType, string commandText, params IDataParameter[] commandParams)
		{
			...
			while (true)
			{
				try
				{
					SqlParameter prm = new SqlParameter("Return Value", null);
		...
		public SqlDataReader ExecuteReader(CommandType commandType, string commandText, int commandTimeout, params IDataParameter[] commandParams)
		{
			...
			while (true)
			{
				try
				{
					...
					int oldTimeout = m_Command.CommandTimeout;
					m_Command.CommandTimeout = commandTimeout;
					SqlDataReader m_Reader = m_Command.ExecuteReader();
					m_Command.CommandTimeout = oldTimeout;
					return m_Reader;
				}
				catch (SqlException sqlEx)
		...
		{
			this.PrepareCommand(commandType, commandText, pageNumber, pageSize, null, commandParams);
			...
			int pageNumber, int pageSize, string sortCol, params IDataParameter[] commandParams)
		{
			...
			System.Text.StringBuilder spSql = new System.Text.StringBuilder();
		...
		private static string ConnectionStringFromTargetDB(TargetDB target)
		{
			...
			switch (target)
			{
				...
				case TargetDB.Eze:
					...
					break;
				default:
					throw new ApplicationException(string.Format("DASQLBase does not know how to resolve {0}", target.ToString()));
			}
		...
		public void DumpParameters()
		{
			...
			char[] charsToTrim = { '/', ' ' };
			System.Diagnostics.Debug.WriteLine(sbNames.ToString().TrimEnd(charsToTrim));
			System.Diagnostics.Debug.WriteLine(sbValues.ToString().TrimEnd(charsToTrim));
		}
		...
		public void DumpDataTable(DataTable dt, int iMaxRows)
		{
			...
			for (int y = 0; y < dt.Columns.Count; y++)
				System.Diagnostics.Debug.Write(dt.Columns[y].ColumnName + "\t");
			System.Diagnostics.Debug.Write("\r\n");

			for (int x = 0; x < iMaxRows; x++)
			{
				for (int y = 0; y < dt.Columns.Count; y++)
					System.Diagnostics.Debug.Write(dt.Rows[x][y] + "\t");
		...
		public void DumpDataRow(DataRow dr, int iMaxRows)
		{
			for (int y = 0; y < dr.Table.Columns.Count; y++)
				System.Diagnostics.Debug.Write(dr.Table.Columns[y].ColumnName + "\t");

			System.Diagnostics.Debug.Write("\r\n");

			for (int y = 0; y < dr.Table.Columns.Count; y++)
				System.Diagnostics.Debug.Write(dr[y] + "\t");
		...
		public void DumpDataRows(DataRow[] dr, int iMaxRows)
		{
			...
			for (int y = 0; y < dr[0].Table.Columns.Count; y++)
				System.Diagnostics.Debug.Write(dr[0].Table.Columns[y].ColumnName + "\t");
			...
			{
				for (int y = 0; y < dr[0].Table.Columns.Count; y++)
					System.Diagnostics.Debug.Write(dr[x][y] + "\t");
