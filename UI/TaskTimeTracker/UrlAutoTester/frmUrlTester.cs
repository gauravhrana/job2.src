...
namespace UrlAutoTester
{
    public partial class frmUrlTester : Form
    {
        ...
        {
			var dt = Framework.Components.Core.SystemEntityXSystemEntityCategoryDataManager.GetBySystemEntityCategory(systemEntityCategoryId.Value, requestProfile);
            if (dt.Rows.Count > 0)
            {   
        ...
        private void LoadSystemEntity()
        {
            ...
            if (systemEntityCategoryData.Count > 0)
            {
                foreach (var dr in systemEntityCategoryData)
                {
                    var childNode              = new TreeNode(Convert.ToString(dr.Name));
                    var systemEntityCategoryId = dr.SystemEntityCategoryId;
			...
			var systemEntityTypeData = Framework.Components.Core.SystemEntityTypeDataManager.Search(SystemEntityTypeDataModel.Empty, requestProfile);
            var xTable = Framework.Components.Core.SystemEntityXSystemEntityCategoryDataManager.Search(SystemEntityXSystemEntityCategoryDataModel.Empty, requestProfile);
