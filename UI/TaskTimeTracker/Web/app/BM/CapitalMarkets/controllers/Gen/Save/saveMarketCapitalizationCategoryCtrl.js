...
angular.module('rootAppShell')
	.controller('saveMarketCapitalizationCategoryCtrl', [
		...
		function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
			...
			$scope.entityName = 'MarketCapitalizationCategory';
			var entityService = $injector.get('marketCapitalizationCategoryService');
			...
			$scope.save = function () {
				...
				$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
					function () {
						...
						if ($scope.WorkflowStateIsNew()) {
							userService.AlertManager.addSuccessAlert('New Record for Market Capitalization Category saved.');
						}
						else{
							userService.AlertManager.addSuccessAlert('Record for Market Capitalization Category saved.');
						}

						$location.url('/' + $scope.entityUrl + '/search');
					},
					function (errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Market Capitalization Category failed to save. ' + [JSON.stringify(errorResponse)]);
					});
			};
			...
			$scope.delete = function () {
				entityService.delete({ detailId: $routeParams.detailId },
					function () {
						userService.AlertManager.addSuccessAlert('Record for Market Capitalization Category deleted.');
						$location.url('/' + $scope.entityUrl + '/search');
					},
					function onFailedDelete(errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Market Capitalization Category failed to delete.' + [JSON.stringify(errorResponse)]);
					});
			};
