	...
	angular.module('rootAppShell')
		.controller('saveFinancialAccountTypeCtrl', [
			...
			function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
				...
				$scope.entityName = 'FinancialAccountType';
				var entityService = $injector.get('financialAccountTypeService');
				...
				$scope.save = function () {
					...
					$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
						function () {
							...
							if ($scope.WorkflowStateIsNew()) {
								userService.AlertManager.addSuccessAlert('New Record for Financial Account Type saved.');
							}
							else{
								userService.AlertManager.addSuccessAlert('Record for Financial Account Type saved.');
							}

							$location.url('/' + $scope.entityUrl + '/search');
						},
						function (errorResponse) {
							userService.AlertManager.addFailureAlert('Record for Financial Account Type failed to save. ' + [JSON.stringify(errorResponse)]);
						});
				};
				...
				$scope.delete = function () {
					entityService.delete({ detailId: $routeParams.detailId },
						function () {
							userService.AlertManager.addSuccessAlert('Record for Financial Account Type deleted.');
							$location.url('/' + $scope.entityUrl + '/search');
						},
						function onFailedDelete(errorResponse) {
							userService.AlertManager.addFailureAlert('Record for Financial Account Type failed to delete.' + [JSON.stringify(errorResponse)]);
						});
				};
