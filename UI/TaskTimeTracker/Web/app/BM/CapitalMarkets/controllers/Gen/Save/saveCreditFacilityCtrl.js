					...
					angular.module('rootAppShell')
						.controller('saveCreditFacilityCtrl', [
							...
							function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
								...
								$scope.entityName = 'CreditFacility';
								var entityService = $injector.get('creditFacilityService');
								...
								$scope.save = function () {
									...
									$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
										function () {
											...
											if ($scope.WorkflowStateIsNew()) {
												userService.AlertManager.addSuccessAlert('New Record for Credit Facility saved.');
											}
											else{
												userService.AlertManager.addSuccessAlert('Record for Credit Facility saved.');
											}

											$location.url('/' + $scope.entityUrl + '/search');
										},
										function (errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Credit Facility failed to save. ' + [JSON.stringify(errorResponse)]);
										});
								};
								...
								$scope.delete = function () {
									entityService.delete({ detailId: $routeParams.detailId },
										function () {
											userService.AlertManager.addSuccessAlert('Record for Credit Facility deleted.');
											$location.url('/' + $scope.entityUrl + '/search');
										},
										function onFailedDelete(errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Credit Facility failed to delete.' + [JSON.stringify(errorResponse)]);
										});
								};
