																	...
																	angular.module('rootAppShell')
																		.controller('saveSecurityXExternalMarketDataIdentifierCtrl', [
																			...
																			function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
																				...
																				$scope.entityName = 'SecurityXExternalMarketDataIdentifier';
																				var entityService = $injector.get('securityXExternalMarketDataIdentifierService');
																				...
																				$scope.save = function () {
																					...
																					$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																						function () {
																							...
																							if ($scope.WorkflowStateIsNew()) {
																								userService.AlertManager.addSuccessAlert('New Record for Security X External Market Data Identifier saved.');
																							}
																							else{
																								userService.AlertManager.addSuccessAlert('Record for Security X External Market Data Identifier saved.');
																							}

																							$location.url('/' + $scope.entityUrl + '/search');
																						},
																						function (errorResponse) {
																							userService.AlertManager.addFailureAlert('Record for Security X External Market Data Identifier failed to save. ' + [JSON.stringify(errorResponse)]);
																						});
																				};
																				...
																				$scope.delete = function () {
																					entityService.delete({ detailId: $routeParams.detailId },
																						function () {
																							userService.AlertManager.addSuccessAlert('Record for Security X External Market Data Identifier deleted.');
																							$location.url('/' + $scope.entityUrl + '/search');
																						},
																						function onFailedDelete(errorResponse) {
																							userService.AlertManager.addFailureAlert('Record for Security X External Market Data Identifier failed to delete.' + [JSON.stringify(errorResponse)]);
																						});
																				};
