								...
								angular.module('rootAppShell')
									.controller('saveInventoryStateCtrl', [
										...
										function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
											...
											$scope.entityName = 'InventoryState';
											var entityService = $injector.get('inventoryStateService');
											...
											$scope.save = function () {
												...
												$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
													function () {
														...
														if ($scope.WorkflowStateIsNew()) {
															userService.AlertManager.addSuccessAlert('New Record for Inventory State saved.');
														}
														else{
															userService.AlertManager.addSuccessAlert('Record for Inventory State saved.');
														}

														$location.url('/' + $scope.entityUrl + '/search');
													},
													function (errorResponse) {
														userService.AlertManager.addFailureAlert('Record for Inventory State failed to save. ' + [JSON.stringify(errorResponse)]);
													});
											};
											...
											$scope.delete = function () {
												entityService.delete({ detailId: $routeParams.detailId },
													function () {
														userService.AlertManager.addSuccessAlert('Record for Inventory State deleted.');
														$location.url('/' + $scope.entityUrl + '/search');
													},
													function onFailedDelete(errorResponse) {
														userService.AlertManager.addFailureAlert('Record for Inventory State failed to delete.' + [JSON.stringify(errorResponse)]);
													});
											};
