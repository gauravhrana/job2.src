	...
	angular.module('rootAppShell')
		.controller('saveAssetTypeCtrl', [
			...
			function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
				...
				$scope.entityName = 'AssetType';
				var entityService = $injector.get('assetTypeService');
				...
				$scope.save = function () {
					...
					$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
						function () {
							...
							if ($scope.WorkflowStateIsNew()) {
								userService.AlertManager.addSuccessAlert('New Record for Asset Type saved.');
							}
							else{
								userService.AlertManager.addSuccessAlert('Record for Asset Type saved.');
							}

							$location.url('/' + $scope.entityUrl + '/search');
						},
						function (errorResponse) {
							userService.AlertManager.addFailureAlert('Record for Asset Type failed to save. ' + [JSON.stringify(errorResponse)]);
						});
				};
				...
				$scope.delete = function () {
					entityService.delete({ detailId: $routeParams.detailId },
						function () {
							userService.AlertManager.addSuccessAlert('Record for Asset Type deleted.');
							$location.url('/' + $scope.entityUrl + '/search');
						},
						function onFailedDelete(errorResponse) {
							userService.AlertManager.addFailureAlert('Record for Asset Type failed to delete.' + [JSON.stringify(errorResponse)]);
						});
				};
