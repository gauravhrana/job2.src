...
angular.module('rootAppShell')
	.controller('saveAccountingParametersCtrl', [
		...
		function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
			...
			$scope.entityName = 'AccountingParameters';
			var entityService = $injector.get('accountingParametersService');
			...
			$scope.save = function () {
				...
				$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
					function () {
						...
						if ($scope.WorkflowStateIsNew()) {
							userService.AlertManager.addSuccessAlert('New Record for Accounting Parameters saved.');
						}
						else{
							userService.AlertManager.addSuccessAlert('Record for Accounting Parameters saved.');
						}

						$location.url('/' + $scope.entityUrl + '/search');
					},
					function (errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Accounting Parameters failed to save. ' + [JSON.stringify(errorResponse)]);
					});
			};
			...
			$scope.delete = function () {
				entityService.delete({ detailId: $routeParams.detailId },
					function () {
						userService.AlertManager.addSuccessAlert('Record for Accounting Parameters deleted.');
						$location.url('/' + $scope.entityUrl + '/search');
					},
					function onFailedDelete(errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Accounting Parameters failed to delete.' + [JSON.stringify(errorResponse)]);
					});
			};
