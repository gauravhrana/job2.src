												...
												angular.module('rootAppShell')
													.controller('saveForwardFXContractCtrl', [
														...
														function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
															...
															$scope.entityName = 'ForwardFXContract';
															var entityService = $injector.get('forwardFXContractService');
															...
															$scope.save = function () {
																...
																$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																	function () {
																		...
																		if ($scope.WorkflowStateIsNew()) {
																			userService.AlertManager.addSuccessAlert('New Record for Forward FX Contract saved.');
																		}
																		else{
																			userService.AlertManager.addSuccessAlert('Record for Forward FX Contract saved.');
																		}

																		$location.url('/' + $scope.entityUrl + '/search');
																	},
																	function (errorResponse) {
																		userService.AlertManager.addFailureAlert('Record for Forward FX Contract failed to save. ' + [JSON.stringify(errorResponse)]);
																	});
															};
															...
															$scope.delete = function () {
																entityService.delete({ detailId: $routeParams.detailId },
																	function () {
																		userService.AlertManager.addSuccessAlert('Record for Forward FX Contract deleted.');
																		$location.url('/' + $scope.entityUrl + '/search');
																	},
																	function onFailedDelete(errorResponse) {
																		userService.AlertManager.addFailureAlert('Record for Forward FX Contract failed to delete.' + [JSON.stringify(errorResponse)]);
																	});
															};
