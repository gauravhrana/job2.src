					...
					angular.module('rootAppShell')
						.controller('saveAutomatedFreezepointCtrl', [
							...
							function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
								...
								$scope.entityName = 'AutomatedFreezepoint';
								var entityService = $injector.get('automatedFreezepointService');
								...
								$scope.save = function () {
									...
									$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
										function () {
											...
											if ($scope.WorkflowStateIsNew()) {
												userService.AlertManager.addSuccessAlert('New Record for Automated Freezepoint saved.');
											}
											else{
												userService.AlertManager.addSuccessAlert('Record for Automated Freezepoint saved.');
											}

											$location.url('/' + $scope.entityUrl + '/search');
										},
										function (errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Automated Freezepoint failed to save. ' + [JSON.stringify(errorResponse)]);
										});
								};
								...
								$scope.delete = function () {
									entityService.delete({ detailId: $routeParams.detailId },
										function () {
											userService.AlertManager.addSuccessAlert('Record for Automated Freezepoint deleted.');
											$location.url('/' + $scope.entityUrl + '/search');
										},
										function onFailedDelete(errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Automated Freezepoint failed to delete.' + [JSON.stringify(errorResponse)]);
										});
								};
