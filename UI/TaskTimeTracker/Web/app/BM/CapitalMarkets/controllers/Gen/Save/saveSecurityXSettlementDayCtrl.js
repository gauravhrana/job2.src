																			...
																			angular.module('rootAppShell')
																				.controller('saveSecurityXSettlementDayCtrl', [
																					...
																					function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
																						...
																						$scope.entityName = 'SecurityXSettlementDay';
																						var entityService = $injector.get('securityXSettlementDayService');
																						...
																						$scope.save = function () {
																							...
																							$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																								function () {
																									...
																									if ($scope.WorkflowStateIsNew()) {
																										userService.AlertManager.addSuccessAlert('New Record for Security X Settlement Day saved.');
																									}
																									else{
																										userService.AlertManager.addSuccessAlert('Record for Security X Settlement Day saved.');
																									}

																									$location.url('/' + $scope.entityUrl + '/search');
																								},
																								function (errorResponse) {
																									userService.AlertManager.addFailureAlert('Record for Security X Settlement Day failed to save. ' + [JSON.stringify(errorResponse)]);
																								});
																						};
																						...
																						$scope.delete = function () {
																							entityService.delete({ detailId: $routeParams.detailId },
																								function () {
																									userService.AlertManager.addSuccessAlert('Record for Security X Settlement Day deleted.');
																									$location.url('/' + $scope.entityUrl + '/search');
																								},
																								function onFailedDelete(errorResponse) {
																									userService.AlertManager.addFailureAlert('Record for Security X Settlement Day failed to delete.' + [JSON.stringify(errorResponse)]);
																								});
																						};
