				...
				angular.module('rootAppShell')
					.controller('saveOrderStatusGroupCtrl', [
						...
						function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
							...
							$scope.entityName = 'OrderStatusGroup';
							var entityService = $injector.get('orderStatusGroupService');
							...
							$scope.save = function () {
								...
								$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
									function () {
										...
										if ($scope.WorkflowStateIsNew()) {
											userService.AlertManager.addSuccessAlert('New Record for Order Status Group saved.');
										}
										else{
											userService.AlertManager.addSuccessAlert('Record for Order Status Group saved.');
										}

										$location.url('/' + $scope.entityUrl + '/search');
									},
									function (errorResponse) {
										userService.AlertManager.addFailureAlert('Record for Order Status Group failed to save. ' + [JSON.stringify(errorResponse)]);
									});
							};
							...
							$scope.delete = function () {
								entityService.delete({ detailId: $routeParams.detailId },
									function () {
										userService.AlertManager.addSuccessAlert('Record for Order Status Group deleted.');
										$location.url('/' + $scope.entityUrl + '/search');
									},
									function onFailedDelete(errorResponse) {
										userService.AlertManager.addFailureAlert('Record for Order Status Group failed to delete.' + [JSON.stringify(errorResponse)]);
									});
							};
