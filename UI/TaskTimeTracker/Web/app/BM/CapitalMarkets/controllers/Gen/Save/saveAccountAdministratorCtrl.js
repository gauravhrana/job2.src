			...
			angular.module('rootAppShell')
				.controller('saveAccountAdministratorCtrl', [
					...
					function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
						...
						$scope.entityName = 'AccountAdministrator';
						var entityService = $injector.get('accountAdministratorService');
						...
						$scope.save = function () {
							...
							$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
								function () {
									...
									if ($scope.WorkflowStateIsNew()) {
										userService.AlertManager.addSuccessAlert('New Record for Account Administrator saved.');
									}
									else{
										userService.AlertManager.addSuccessAlert('Record for Account Administrator saved.');
									}

									$location.url('/' + $scope.entityUrl + '/search');
								},
								function (errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Account Administrator failed to save. ' + [JSON.stringify(errorResponse)]);
								});
						};
						...
						$scope.delete = function () {
							entityService.delete({ detailId: $routeParams.detailId },
								function () {
									userService.AlertManager.addSuccessAlert('Record for Account Administrator deleted.');
									$location.url('/' + $scope.entityUrl + '/search');
								},
								function onFailedDelete(errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Account Administrator failed to delete.' + [JSON.stringify(errorResponse)]);
								});
						};
