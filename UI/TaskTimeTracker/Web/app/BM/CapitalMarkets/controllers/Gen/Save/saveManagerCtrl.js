									...
									angular.module('rootAppShell')
										.controller('saveManagerCtrl', [
											...
											function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
												...
												$scope.entityName = 'Manager';
												var entityService = $injector.get('managerService');
												...
												$scope.save = function () {
													...
													$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
														function () {
															...
															if ($scope.WorkflowStateIsNew()) {
																userService.AlertManager.addSuccessAlert('New Record for Manager saved.');
															}
															else{
																userService.AlertManager.addSuccessAlert('Record for Manager saved.');
															}

															$location.url('/' + $scope.entityUrl + '/search');
														},
														function (errorResponse) {
															userService.AlertManager.addFailureAlert('Record for Manager failed to save. ' + [JSON.stringify(errorResponse)]);
														});
												};
												...
												$scope.delete = function () {
													entityService.delete({ detailId: $routeParams.detailId },
														function () {
															userService.AlertManager.addSuccessAlert('Record for Manager deleted.');
															$location.url('/' + $scope.entityUrl + '/search');
														},
														function onFailedDelete(errorResponse) {
															userService.AlertManager.addFailureAlert('Record for Manager failed to delete.' + [JSON.stringify(errorResponse)]);
														});
												};
