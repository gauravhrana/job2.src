												...
												angular.module('rootAppShell')
													.controller('saveWithholdingTaxTypeCtrl', [
														...
														function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
															...
															$scope.entityName = 'WithholdingTaxType';
															var entityService = $injector.get('withholdingTaxTypeService');
															...
															$scope.save = function () {
																...
																$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																	function () {
																		...
																		if ($scope.WorkflowStateIsNew()) {
																			userService.AlertManager.addSuccessAlert('New Record for Withholding Tax Type saved.');
																		}
																		else{
																			userService.AlertManager.addSuccessAlert('Record for Withholding Tax Type saved.');
																		}

																		$location.url('/' + $scope.entityUrl + '/search');
																	},
																	function (errorResponse) {
																		userService.AlertManager.addFailureAlert('Record for Withholding Tax Type failed to save. ' + [JSON.stringify(errorResponse)]);
																	});
															};
															...
															$scope.delete = function () {
																entityService.delete({ detailId: $routeParams.detailId },
																	function () {
																		userService.AlertManager.addSuccessAlert('Record for Withholding Tax Type deleted.');
																		$location.url('/' + $scope.entityUrl + '/search');
																	},
																	function onFailedDelete(errorResponse) {
																		userService.AlertManager.addFailureAlert('Record for Withholding Tax Type failed to delete.' + [JSON.stringify(errorResponse)]);
																	});
															};
