													...
													angular.module('rootAppShell')
														.controller('saveFinancialAccountClassCtrl', [
															...
															function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
																...
																$scope.entityName = 'FinancialAccountClass';
																var entityService = $injector.get('financialAccountClassService');
																...
																$scope.save = function () {
																	...
																	$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																		function () {
																			...
																			if ($scope.WorkflowStateIsNew()) {
																				userService.AlertManager.addSuccessAlert('New Record for Financial Account Class saved.');
																			}
																			else{
																				userService.AlertManager.addSuccessAlert('Record for Financial Account Class saved.');
																			}

																			$location.url('/' + $scope.entityUrl + '/search');
																		},
																		function (errorResponse) {
																			userService.AlertManager.addFailureAlert('Record for Financial Account Class failed to save. ' + [JSON.stringify(errorResponse)]);
																		});
																};
																...
																$scope.delete = function () {
																	entityService.delete({ detailId: $routeParams.detailId },
																		function () {
																			userService.AlertManager.addSuccessAlert('Record for Financial Account Class deleted.');
																			$location.url('/' + $scope.entityUrl + '/search');
																		},
																		function onFailedDelete(errorResponse) {
																			userService.AlertManager.addFailureAlert('Record for Financial Account Class failed to delete.' + [JSON.stringify(errorResponse)]);
																		});
																};
