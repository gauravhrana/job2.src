					...
					angular.module('rootAppShell')
						.controller('saveOrderStatusTypeCtrl', [
							...
							function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
								...
								$scope.entityName = 'OrderStatusType';
								var entityService = $injector.get('orderStatusTypeService');
								...
								dataAutoCompleteService.getComboSource({ methodName: 'GetOrderStatusGroupList' },
								function (response) {
									$scope.OrderStatusGroupList = response;
									$scope.selectedOrderStatusGroupItem = response[0];
								}, onFailedLoad);
								...
								function onSuccessLoad(data) {
									...
									var itemsOrderStatusGroup = jQuery.grep($scope.OrderStatusGroupList, function (a) {
										return a.OrderStatusGroupId == data.OrderStatusGroupId
									});
									$scope.selectedOrderStatusGroupItem = itemsOrderStatusGroup[0];
								}
								...
								$scope.save = function () {
									...
									$scope.EntityItem.OrderStatusGroupId = $scope.selectedOrderStatusGroupItem.OrderStatusGroupId;
									...
									$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
										function () {
											...
											if ($scope.WorkflowStateIsNew()) {
												userService.AlertManager.addSuccessAlert('New Record for Order Status Type saved.');
											}
											else{
												userService.AlertManager.addSuccessAlert('Record for Order Status Type saved.');
											}

											$location.url('/' + $scope.entityUrl + '/search');
										},
										function (errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Order Status Type failed to save. ' + [JSON.stringify(errorResponse)]);
										});
								};
								...
								$scope.delete = function () {
									entityService.delete({ detailId: $routeParams.detailId },
										function () {
											userService.AlertManager.addSuccessAlert('Record for Order Status Type deleted.');
											$location.url('/' + $scope.entityUrl + '/search');
										},
										function onFailedDelete(errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Order Status Type failed to delete.' + [JSON.stringify(errorResponse)]);
										});
								};
