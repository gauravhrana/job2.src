														...
														angular.module('rootAppShell')
															.controller('saveAdjustmentCategoryCtrl', [
																...
																function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
																	...
																	$scope.entityName = 'AdjustmentCategory';
																	var entityService = $injector.get('adjustmentCategoryService');
																	...
																	$scope.save = function () {
																		...
																		$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
																			function () {
																				...
																				if ($scope.WorkflowStateIsNew()) {
																					userService.AlertManager.addSuccessAlert('New Record for Adjustment Category saved.');
																				}
																				else{
																					userService.AlertManager.addSuccessAlert('Record for Adjustment Category saved.');
																				}

																				$location.url('/' + $scope.entityUrl + '/search');
																			},
																			function (errorResponse) {
																				userService.AlertManager.addFailureAlert('Record for Adjustment Category failed to save. ' + [JSON.stringify(errorResponse)]);
																			});
																	};
																	...
																	$scope.delete = function () {
																		entityService.delete({ detailId: $routeParams.detailId },
																			function () {
																				userService.AlertManager.addSuccessAlert('Record for Adjustment Category deleted.');
																				$location.url('/' + $scope.entityUrl + '/search');
																			},
																			function onFailedDelete(errorResponse) {
																				userService.AlertManager.addFailureAlert('Record for Adjustment Category failed to delete.' + [JSON.stringify(errorResponse)]);
																			});
																	};
