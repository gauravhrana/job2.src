						...
						angular.module('rootAppShell')
							.controller('saveCreditContractCtrl', [
								...
								function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
									...
									$scope.entityName = 'CreditContract';
									var entityService = $injector.get('creditContractService');
									...
									$scope.save = function () {
										...
										$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
											function () {
												...
												if ($scope.WorkflowStateIsNew()) {
													userService.AlertManager.addSuccessAlert('New Record for Credit Contract saved.');
												}
												else{
													userService.AlertManager.addSuccessAlert('Record for Credit Contract saved.');
												}

												$location.url('/' + $scope.entityUrl + '/search');
											},
											function (errorResponse) {
												userService.AlertManager.addFailureAlert('Record for Credit Contract failed to save. ' + [JSON.stringify(errorResponse)]);
											});
									};
									...
									$scope.delete = function () {
										entityService.delete({ detailId: $routeParams.detailId },
											function () {
												userService.AlertManager.addSuccessAlert('Record for Credit Contract deleted.');
												$location.url('/' + $scope.entityUrl + '/search');
											},
											function onFailedDelete(errorResponse) {
												userService.AlertManager.addFailureAlert('Record for Credit Contract failed to delete.' + [JSON.stringify(errorResponse)]);
											});
									};
