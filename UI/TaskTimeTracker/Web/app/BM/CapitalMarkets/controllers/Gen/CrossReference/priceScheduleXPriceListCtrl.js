...
angular.module('rootAppShell')
	.controller('priceScheduleXPriceListCtrl', [
		'$location', '$scope', '$injector', '$routeParams', 'userService', 'priceScheduleXPriceListService',
		function ($location, $scope, $injector, $routeParams, userService, priceScheduleXPriceListService) {

			$scope.entityDisplayName = 'PriceSchedule X PriceList';

			$scope.sourceEntity = 'PriceSchedule';
			...
			$scope.resetSourceEntity = function () {
				if ($scope.sourceEntity == 'PriceSchedule') {
					$scope.targetEntity = 'PriceList';
				} else {
					$scope.targetEntity = 'PriceSchedule';
				}
				// get source entity items
				priceScheduleXPriceListService.getSourceEntityList({ value: JSON.stringify($scope.sourceEntity) }, onSuccessLoadSourceEntityList, onFailedLoad);
			};

			$scope.resetSourceEntity();

			$scope.loadTargetEntity = function () {
				priceScheduleXPriceListService.getSourceEntityList(
					{
			...
			function onSuccessLoadTargetEntityList(data) {
				...
				priceScheduleXPriceListService.getEntityRecords(
					{
		...
		$scope.saveEntityRecords = function () {
			priceScheduleXPriceListService.removeEntityRecords(
				{
					value: JSON.stringify($scope.sourceEntity)
					, value1: JSON.stringify($scope.sourceEntityItem.EntityKey)
				},
			...
			function onSuccessRemoveRecords(data) {
				if (data.Result == true) {
					if ($scope.assignedTargetEntityList.length > 0) {
						...
						priceScheduleXPriceListService.addEntityRecords(
						{
