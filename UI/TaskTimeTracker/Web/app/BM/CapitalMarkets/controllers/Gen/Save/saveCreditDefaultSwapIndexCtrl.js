			...
			angular.module('rootAppShell')
				.controller('saveCreditDefaultSwapIndexCtrl', [
					...
					function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
						...
						$scope.entityName = 'CreditDefaultSwapIndex';
						var entityService = $injector.get('creditDefaultSwapIndexService');
						...
						$scope.save = function () {
							...
							$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
								function () {
									...
									if ($scope.WorkflowStateIsNew()) {
										userService.AlertManager.addSuccessAlert('New Record for Credit Default Swap Index saved.');
									}
									else{
										userService.AlertManager.addSuccessAlert('Record for Credit Default Swap Index saved.');
									}

									$location.url('/' + $scope.entityUrl + '/search');
								},
								function (errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Credit Default Swap Index failed to save. ' + [JSON.stringify(errorResponse)]);
								});
						};
						...
						$scope.delete = function () {
							entityService.delete({ detailId: $routeParams.detailId },
								function () {
									userService.AlertManager.addSuccessAlert('Record for Credit Default Swap Index deleted.');
									$location.url('/' + $scope.entityUrl + '/search');
								},
								function onFailedDelete(errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Credit Default Swap Index failed to delete.' + [JSON.stringify(errorResponse)]);
								});
						};
