...
angular.module('rootAppShell')
	.controller('saveFieldConfigurationBaseCtrl', [
		...
		function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
			...
			$scope.entityName = 'FieldConfigurationBase';
			var entityService = $injector.get('fieldConfigurationBaseService');
			...
			$scope.save = function () {
				...
				$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
					function () {
						...
						if ($scope.WorkflowStateIsNew()) {
							userService.AlertManager.addSuccessAlert('New Record for Field Configuration Base saved.');
						}
						else{
							userService.AlertManager.addSuccessAlert('Record for Field Configuration Base saved.');
						}

						$location.url('/' + $scope.entityUrl + '/search');
					},
					function (errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Field Configuration Base failed to save. ' + [JSON.stringify(errorResponse)]);
					});
			};
			...
			$scope.delete = function () {
				entityService.delete({ detailId: $routeParams.detailId },
					function () {
						userService.AlertManager.addSuccessAlert('Record for Field Configuration Base deleted.');
						$location.url('/' + $scope.entityUrl + '/search');
					},
					function onFailedDelete(errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Field Configuration Base failed to delete.' + [JSON.stringify(errorResponse)]);
					});
			};
