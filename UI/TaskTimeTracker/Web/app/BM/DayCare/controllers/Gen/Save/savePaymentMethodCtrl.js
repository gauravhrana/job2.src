					...
					angular.module('rootAppShell')
						.controller('savePaymentMethodCtrl', [
							...
							function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
								...
								$scope.entityName = 'PaymentMethod';
								var entityService = $injector.get('paymentMethodService');
								...
								$scope.save = function () {
									...
									$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
										function () {
											...
											if ($scope.WorkflowStateIsNew()) {
												userService.AlertManager.addSuccessAlert('New Record for Payment Method saved.');
											}
											else{
												userService.AlertManager.addSuccessAlert('Record for Payment Method saved.');
											}

											$location.url('/' + $scope.entityUrl + '/search');
										},
										function (errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Payment Method failed to save. ' + [JSON.stringify(errorResponse)]);
										});
								};
								...
								$scope.delete = function () {
									entityService.delete({ detailId: $routeParams.detailId },
										function () {
											userService.AlertManager.addSuccessAlert('Record for Payment Method deleted.');
											$location.url('/' + $scope.entityUrl + '/search');
										},
										function onFailedDelete(errorResponse) {
											userService.AlertManager.addFailureAlert('Record for Payment Method failed to delete.' + [JSON.stringify(errorResponse)]);
										});
								};
