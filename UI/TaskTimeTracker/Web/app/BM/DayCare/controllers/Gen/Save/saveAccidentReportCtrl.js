		...
		angular.module('rootAppShell')
			.controller('saveAccidentReportCtrl', [
				...
				function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
					...
					$scope.entityName = 'AccidentReport';
					var entityService = $injector.get('accidentReportService');
					...
					$scope.save = function () {
						...
						$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
							function () {
								...
								if ($scope.WorkflowStateIsNew()) {
									userService.AlertManager.addSuccessAlert('New Record for Accident Report saved.');
								}
								else{
									userService.AlertManager.addSuccessAlert('Record for Accident Report saved.');
								}

								$location.url('/' + $scope.entityUrl + '/search');
							},
							function (errorResponse) {
								userService.AlertManager.addFailureAlert('Record for Accident Report failed to save. ' + [JSON.stringify(errorResponse)]);
							});
					};
					...
					$scope.delete = function () {
						entityService.delete({ detailId: $routeParams.detailId },
							function () {
								userService.AlertManager.addSuccessAlert('Record for Accident Report deleted.');
								$location.url('/' + $scope.entityUrl + '/search');
							},
							function onFailedDelete(errorResponse) {
								userService.AlertManager.addFailureAlert('Record for Accident Report failed to delete.' + [JSON.stringify(errorResponse)]);
							});
					};
