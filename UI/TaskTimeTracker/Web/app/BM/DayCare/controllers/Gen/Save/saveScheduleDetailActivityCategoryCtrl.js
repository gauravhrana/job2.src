		...
		angular.module('rootAppShell')
			.controller('saveScheduleDetailActivityCategoryCtrl', [
				...
				function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
					...
					$scope.save = function () {
						...
						$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
							function () {
								...
								if ($scope.WorkflowStateIsNew()) {
									userService.AlertManager.addSuccessAlert('New Record for Schedule Detail Activity Category saved.');
								}
								else{
									userService.AlertManager.addSuccessAlert('Record for Schedule Detail Activity Category saved.');
								}

								$location.url('/' + $scope.entityUrl + '/search');
							},
							function (errorResponse) {
								userService.AlertManager.addFailureAlert('Record for Schedule Detail Activity Category failed to save. ' + [JSON.stringify(errorResponse)]);
							});
					};
					...
					$scope.delete = function () {
						entityService.delete({ detailId: $routeParams.detailId },
							function () {
								userService.AlertManager.addSuccessAlert('Record for Schedule Detail Activity Category deleted.');
								$location.url('/' + $scope.entityUrl + '/search');
							},
							function onFailedDelete(errorResponse) {
								userService.AlertManager.addFailureAlert('Record for Schedule Detail Activity Category failed to delete.' + [JSON.stringify(errorResponse)]);
							});
					};
