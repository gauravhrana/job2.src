...
angular.module('rootAppShell')
	.controller('saveCourseInstanceCtrl', [
		...
		function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
			...
			$scope.save = function () {
				...
				$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
					function () {
						...
						if ($scope.WorkflowStateIsNew()) {
							userService.AlertManager.addSuccessAlert('New Record for Course Instance saved.');
						}
						else{
							userService.AlertManager.addSuccessAlert('Record for Course Instance saved.');
						}

						$location.url('/' + $scope.entityUrl + '/search');
					},
					function (errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Course Instance failed to save. ' + [JSON.stringify(errorResponse)]);
					});
			};
			...
			$scope.delete = function () {
				entityService.delete({ detailId: $routeParams.detailId },
					function () {
						userService.AlertManager.addSuccessAlert('Record for Course Instance deleted.');
						$location.url('/' + $scope.entityUrl + '/search');
					},
					function onFailedDelete(errorResponse) {
						userService.AlertManager.addFailureAlert('Record for Course Instance failed to delete.' + [JSON.stringify(errorResponse)]);
					});
			};
