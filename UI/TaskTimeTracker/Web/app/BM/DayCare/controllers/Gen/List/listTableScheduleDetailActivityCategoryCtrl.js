		...
		angular.module('rootAppShell')
			.controller('listTableScheduleDetailActivityCategoryCtrl', [
				'$scope', '$injector', '$routeParams', '$location', 'fieldConfigurationService', 'autoCompleteDataService',
				'dataAutoCompleteService', 'userService', 'userPreferenceUtilityService',

				function ($scope, $injector, $routeParams, $location, fieldConfigurationService, autoCompleteDataService, 
				dataAutoCompleteService, userService, userPreferenceUtilityService) {

					$scope.entityName = 'ScheduleDetailActivityCategory';
					var entityService = $injector.get('scheduleDetailActivityCategoryService');
					...
					$scope.PrimaryKey = 'ScheduleDetailActivityCategoryId';
					...
					userPreferenceUtilityService.getUserDateFormat(null,
					    function (response) {
					        $scope.userDateFormat = response.Result;
					    }, onFailedLoad);

					// get pre filled Date Items
					dataAutoCompleteService.getPreFilledDateItemsList(null,
					    function (response) {
					        $scope.preFilledDateItemsList = response;
					    }, onFailedLoad);

					$scope.fillupDate = function (fcSearchItem) {
					    dataAutoCompleteService.getDatesFromPreFilledItem({ value: fcSearchItem.PreFilledDateItem },
					    function (response) {
					        fcSearchItem.fromDate = response[0];
					        fcSearchItem.toDate = response[1];
					    }, onFailedLoad);
					}
					...
					function onSuccessLoadSearchFC(data) {
						...
						for (var i = 0; i < data.length; i++) {
							...
							// date panel case
							if (controlType == 'DatePanel') {

								var fromDateValue = '';
								var toDateValue = '';
								var preFilledValue = 'None';

								var dateValue = data[i].Value;
								if (dateValue != '') {
									var tmpArr = dateValue.split('&');

									fromDateValue = tmpArr[0];
									toDateValue = tmpArr[1];
									preFilledValue = tmpArr[2];
								}
								...
								var searchItem = {  
									...
									,   Value: data[i].Value
									,   PreFilledDateItem: preFilledValue
									,   fromDate: fromDateValue
									,   toDate: toDateValue
								};

								searchItem.ControlType = 'datePanel';

								$scope.searchFieldConfigurations.push(searchItem);

								// fill according to pre filled date item
								if (preFilledValue != 'Custom') {
									$scope.fillupDate(searchItem);
								}
								...
								if (columnName == 'GroupBy' || columnName == 'SubGroupBy') {
									...
									if (methodInfo.Found == true) {
										...
										if (methodInfo.EntityName != undefined) {
											entityName = methodInfo.EntityName;
										}

										if (methodInfo.ModuleName != undefined || methodInfo.ModuleName != '') {
											moduleName = methodInfo.ModuleName;
										}

										searchItem.URL = 'Home#/' + moduleName + '/' + entityName + '/search'
					...
					$scope.searchData = function () {
						...
						for (var i = 0; i < $scope.searchFieldConfigurations.length; i++) {
							if ($scope.searchFieldConfigurations[i].ControlType != 'datePanel') {
								...
								var x = new Date($scope.searchFieldConfigurations[i].fromDate);
								var y = new Date($scope.searchFieldConfigurations[i].toDate);

								var filterName = $scope.searchFieldConfigurations[i].Name;
								searchObj[filterName] = $scope.searchFieldConfigurations[i].fromDate + '&' + 
								                        $scope.searchFieldConfigurations[i].toDate + '&' +
								                        $scope.searchFieldConfigurations[i].PreFilledDateItem + '&';
							}
					...
					function onSuccessLoad(data) {
						...
						if ($scope.groupByColumn != -1  &&  $scope.groupByColumn != undefined) {
