			...
			angular.module('rootAppShell')
				.controller('saveGeographicRegionCtrl', [
					...
					function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService, $cookies) {
						...
						$scope.entityName = 'GeographicRegion';
						var entityService = $injector.get('geographicRegionService');
						...
						$scope.save = function () {
							...
							$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
								function () {
									...
									if ($scope.WorkflowStateIsNew()) {
										userService.AlertManager.addSuccessAlert('New Record for Geographic Region saved.');
									}
									else{
										userService.AlertManager.addSuccessAlert('Record for Geographic Region saved.');
									}

									$location.url('/' + $scope.entityUrl + '/search');
								},
								function (errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Geographic Region failed to save. ' + [JSON.stringify(errorResponse)]);
								});
						};
						...
						$scope.delete = function () {
							entityService.delete({ detailId: $routeParams.detailId },
								function () {
									userService.AlertManager.addSuccessAlert('Record for Geographic Region deleted.');
									$location.url('/' + $scope.entityUrl + '/search');
								},
								function onFailedDelete(errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Geographic Region failed to delete.' + [JSON.stringify(errorResponse)]);
								});
						};
