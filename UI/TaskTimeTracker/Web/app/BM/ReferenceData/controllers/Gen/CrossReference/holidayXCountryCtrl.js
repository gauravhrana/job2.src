...
angular.module('rootAppShell')
	.controller('holidayXCountryCtrl', [
		'$location', '$scope', '$injector', '$routeParams', 'userService', 'holidayXCountryService',
		function ($location, $scope, $injector, $routeParams, userService, holidayXCountryService) {

			$scope.entityDisplayName = 'Holiday X Country';

			$scope.sourceEntity = 'Holiday';
			...
			$scope.resetSourceEntity = function () {
				if ($scope.sourceEntity == 'Holiday') {
					$scope.targetEntity = 'Country';
				} else {
					$scope.targetEntity = 'Holiday';
				}
				// get source entity items
				holidayXCountryService.getSourceEntityList({ value: JSON.stringify($scope.sourceEntity) }, onSuccessLoadSourceEntityList, onFailedLoad);
			};

			$scope.resetSourceEntity();

			$scope.loadTargetEntity = function () {
				holidayXCountryService.getSourceEntityList(
					{
			...
			function onSuccessLoadTargetEntityList(data) {
				...
				holidayXCountryService.getEntityRecords(
					{
		...
		$scope.saveEntityRecords = function () {
			holidayXCountryService.removeEntityRecords(
				{
					value: JSON.stringify($scope.sourceEntity)
					, value1: JSON.stringify($scope.sourceEntityItem.EntityKey)
				},
			...
			function onSuccessRemoveRecords(data) {
				if (data.Result == true) {
					if ($scope.assignedTargetEntityList.length > 0) {
						...
						holidayXCountryService.addEntityRecords(
						{
