...
angular.module('rootAppShell')
	.controller('timeZoneXCountryCtrl', [
		'$location', '$scope', '$injector', '$routeParams', 'userService', 'timeZoneXCountryService',
		function ($location, $scope, $injector, $routeParams, userService, timeZoneXCountryService) {

			$scope.entityDisplayName = 'TimeZone X Country';

			$scope.sourceEntity = 'TimeZone';
			...
			$scope.resetSourceEntity = function () {
				if ($scope.sourceEntity == 'TimeZone') {
					$scope.targetEntity = 'Country';
				} else {
					$scope.targetEntity = 'TimeZone';
				}
				// get source entity items
				timeZoneXCountryService.getSourceEntityList({ value: JSON.stringify($scope.sourceEntity) }, onSuccessLoadSourceEntityList, onFailedLoad);
			};

			$scope.resetSourceEntity();

			$scope.loadTargetEntity = function () {
				timeZoneXCountryService.getSourceEntityList(
					{
			...
			function onSuccessLoadTargetEntityList(data) {
				...
				timeZoneXCountryService.getEntityRecords(
					{
		...
		$scope.saveEntityRecords = function () {
			timeZoneXCountryService.removeEntityRecords(
				{
					value: JSON.stringify($scope.sourceEntity)
					, value1: JSON.stringify($scope.sourceEntityItem.EntityKey)
				},
			...
			function onSuccessRemoveRecords(data) {
				if (data.Result == true) {
					if ($scope.assignedTargetEntityList.length > 0) {
						...
						timeZoneXCountryService.addEntityRecords(
						{
