			...
			angular.module('rootAppShell')
				.controller('saveCounselCtrl', [
					...
					function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
						...
						$scope.entityName = 'Counsel';
						var entityService = $injector.get('counselService');
						...
						$scope.save = function () {
							...
							$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
								function () {
									...
									if ($scope.WorkflowStateIsNew()) {
										userService.AlertManager.addSuccessAlert('New Record for Counsel saved.');
									}
									else{
										userService.AlertManager.addSuccessAlert('Record for Counsel saved.');
									}

									$location.url('/' + $scope.entityUrl + '/search');
								},
								function (errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Counsel failed to save. ' + [JSON.stringify(errorResponse)]);
								});
						};
						...
						$scope.delete = function () {
							entityService.delete({ detailId: $routeParams.detailId },
								function () {
									userService.AlertManager.addSuccessAlert('Record for Counsel deleted.');
									$location.url('/' + $scope.entityUrl + '/search');
								},
								function onFailedDelete(errorResponse) {
									userService.AlertManager.addFailureAlert('Record for Counsel failed to delete.' + [JSON.stringify(errorResponse)]);
								});
						};
