				...
				angular.module('rootAppShell')
					.controller('saveJurisdictionsCtrl', [
						...
						function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
							...
							$scope.entityName = 'Jurisdictions';
							var entityService = $injector.get('jurisdictionsService');
							...
							$scope.save = function () {
								...
								$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
									function () {
										...
										if ($scope.WorkflowStateIsNew()) {
											userService.AlertManager.addSuccessAlert('New Record for Jurisdictions saved.');
										}
										else{
											userService.AlertManager.addSuccessAlert('Record for Jurisdictions saved.');
										}

										$location.url('/' + $scope.entityUrl + '/search');
									},
									function (errorResponse) {
										userService.AlertManager.addFailureAlert('Record for Jurisdictions failed to save. ' + [JSON.stringify(errorResponse)]);
									});
							};
							...
							$scope.delete = function () {
								entityService.delete({ detailId: $routeParams.detailId },
									function () {
										userService.AlertManager.addSuccessAlert('Record for Jurisdictions deleted.');
										$location.url('/' + $scope.entityUrl + '/search');
									},
									function onFailedDelete(errorResponse) {
										userService.AlertManager.addFailureAlert('Record for Jurisdictions failed to delete.' + [JSON.stringify(errorResponse)]);
									});
							};
