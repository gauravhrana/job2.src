							...
							angular.module('rootAppShell')
								.controller('saveReportingRequirementCtrl', [
									...
									function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
										...
										$scope.entityName = 'ReportingRequirement';
										var entityService = $injector.get('reportingRequirementService');
										...
										$scope.save = function () {
											...
											$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
												function () {
													...
													if ($scope.WorkflowStateIsNew()) {
														userService.AlertManager.addSuccessAlert('New Record for Reporting Requirement saved.');
													}
													else{
														userService.AlertManager.addSuccessAlert('Record for Reporting Requirement saved.');
													}

													$location.url('/' + $scope.entityUrl + '/search');
												},
												function (errorResponse) {
													userService.AlertManager.addFailureAlert('Record for Reporting Requirement failed to save. ' + [JSON.stringify(errorResponse)]);
												});
										};
										...
										$scope.delete = function () {
											entityService.delete({ detailId: $routeParams.detailId },
												function () {
													userService.AlertManager.addSuccessAlert('Record for Reporting Requirement deleted.');
													$location.url('/' + $scope.entityUrl + '/search');
												},
												function onFailedDelete(errorResponse) {
													userService.AlertManager.addFailureAlert('Record for Reporting Requirement failed to delete.' + [JSON.stringify(errorResponse)]);
												});
										};
