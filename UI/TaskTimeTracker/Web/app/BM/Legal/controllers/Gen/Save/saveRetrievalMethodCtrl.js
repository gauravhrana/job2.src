									...
									angular.module('rootAppShell')
										.controller('saveRetrievalMethodCtrl', [
											...
											function (dataAutoCompleteService, $location, $scope, $injector, $routeParams, userService) {
												...
												$scope.entityName = 'RetrievalMethod';
												var entityService = $injector.get('retrievalMethodService');
												...
												$scope.save = function () {
													...
													$scope.EntityItem = entityService[saveMethod]($scope.EntityItem,
														function () {
															...
															if ($scope.WorkflowStateIsNew()) {
																userService.AlertManager.addSuccessAlert('New Record for Retrieval Method saved.');
															}
															else{
																userService.AlertManager.addSuccessAlert('Record for Retrieval Method saved.');
															}

															$location.url('/' + $scope.entityUrl + '/search');
														},
														function (errorResponse) {
															userService.AlertManager.addFailureAlert('Record for Retrieval Method failed to save. ' + [JSON.stringify(errorResponse)]);
														});
												};
												...
												$scope.delete = function () {
													entityService.delete({ detailId: $routeParams.detailId },
														function () {
															userService.AlertManager.addSuccessAlert('Record for Retrieval Method deleted.');
															$location.url('/' + $scope.entityUrl + '/search');
														},
														function onFailedDelete(errorResponse) {
															userService.AlertManager.addFailureAlert('Record for Retrieval Method failed to delete.' + [JSON.stringify(errorResponse)]);
														});
												};
