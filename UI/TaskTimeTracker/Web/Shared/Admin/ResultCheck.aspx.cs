...
namespace Shared.UI.Web.Admin
{
	public partial class ResultCheck : BasePage
	{
				...
				if (drpApplicationId.SelectedItem.Value.Equals("All"))
				{
					...
					for (var i = 0; i < dt.Rows.Count; i++)
					{
						if (!dt.Rows[i][ApplicationDataModel.DataColumns.Code].ToString().Equals(""))
						{
							...
							var applicationName = dt.Rows[i][ApplicationDataModel.DataColumns.Name] + " RN";
		...
		public ResultSet CalculateRNandSDWorkedHours(string emailAddress, int appId)
		{
			...
			var format = oDateRange.DateTimeFormat;
			...
			if(appId.Equals("All"))
			{
				var allObj = new ReleaseLogDetailDataModel();
				allObj.ReleaseDateMin = Convert.ToDateTime(fromDate);
				allObj.ReleaseDateMax = Convert.ToDateTime(toDate);

				var dt = ReleaseLogDetailDataManager.Search(allObj, SessionVariables.RequestProfile);

				for (var i = 0; i < dt.Rows.Count; i++)
				{
					if (dt.Rows[i][ReleaseLogDetailDataModel.DataColumns.PrimaryDeveloper].Equals(developer) || dt.Rows[i][ReleaseLogDetailDataModel.DataColumns.PrimaryDeveloper].Equals("Admin"))
					{
						temp = (double)Convert.ToDouble(dt.Rows[i][ReleaseLogDetailDataModel.DataColumns.TimeSpent]);
						rnTotalHours = rnTotalHours + temp;
					}
			...
			else
			{
				...
				if (sDt.Rows.Count > 0)
				{
					...
					obj2.PersonId = (int)(sDt.Rows[0][ApplicationUserDataModel.DataColumns.ApplicationUserId]);
		...
		public void CalculateRNandSDWorkedHoursForAllUsersAllApps()
		{
			...
			var appWiseResult = new Dictionary<string, ResultSet>();
			...
			for (var i = 0; i < applicationUserList.Rows.Count; i++)
			{
				...
				for (var i = 0; i < applicationList.Rows.Count; i++)
				{
					...
					{
						gv.Columns.Add(new BoundField());
						var applicationName = applicationList.Rows[i][ApplicationDataModel.DataColumns.Name] + " RN";
						gv.Columns[cell].HeaderText = applicationName.ToString();
						cell++;
					}
				}
