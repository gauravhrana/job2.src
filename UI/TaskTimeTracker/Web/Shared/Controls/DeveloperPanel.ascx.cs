...
namespace Shared.UI.Web.Controls
{
	...
	
	{
		...
		protected void drpApplicationMode_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			PreferenceUtility.UpdateUserPreference("General", ApplicationCommon.UserApplicationModeId, drpApplicationMode2.SelectedValue);
		...
		{
			var path = InformationCommon.CurrentExecutionPath(Request.CurrentExecutionFilePath);
			var filenames = path.Split('/');
			labelPath.Text = Server.MapPath(".") + "\\" + filenames[filenames.Length - 1];
			...
			drpGridLines.SelectedValue = PreferenceUtility.GetUserPreferenceByKey(ApplicationCommon.SearchFilterGridLinesKey);
		...
		protected void drpMenuCategory_SelectedIndexChanged(object sender, EventArgs e)
		{
			...
			PreferenceUtility.UpdateUserPreference("General", ApplicationCommon.UserMenuCategory, menuCategory);
		...
		protected void lnkHelp_Click(object sender, EventArgs e)
		{
			...
			if (dt != null)
			{
                var helpPageId = dt.HelpPageId.Value;
