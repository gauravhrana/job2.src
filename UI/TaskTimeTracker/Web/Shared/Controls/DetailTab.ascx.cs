...
namespace Shared.UI.Web.Controls
{
	public partial class DetailTabControl : BaseControl
	{
		...
		public void Setup(string userPreferenceCategory, bool isBindAllTabs = true)
		{
			...
			TabOrientation = PreferenceUtility.GetUserPreferenceByKeyAsTabOrientation(ApplicationCommon.TabOrientation, SettingCategory);

			hdnTabOrientation.Value = TabOrientation.ToString();

			IsAllTab = PreferenceUtility.GetUserPreferenceByKeyAsBoolean(ApplicationCommon.AllTabExists, SettingCategory);

			if (IsAllTab)
			{
				IsAllTabSelected = PreferenceUtility.GetUserPreferenceByKeyAsBoolean(ApplicationCommon.AllTabSelected, SettingCategory);
			}

			if (TabOrientation == TabOrientation.Vertical)
			{
				//divTabContainer.Visible = false;
				TabHeaderBackgroundColor = PreferenceUtility.GetApplicationUserPreferenceByKey(ApplicationCommon.TabHeaderBackgroundColor);
			}
			else
			{
				if (IsAllTab)
