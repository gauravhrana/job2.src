...
namespace Shared.WebCommon.UI.Web
{
	public class InformationCommon
	{
		...
		public static string CurrentExecutionPath(string executionPath)
		{
			...
			//data.RouteName = "ScheduleEntityRouteSearch";
			//var entityExists = ApplicationRouteDataManager.GetDetails(data, SessionVariables.RequestProfile);
			
			//if(entityExists != null)
			//{
			//	data.RouteName= paths[1] + "EntityRouteSearch";

			//	return entityExists.RelativeRoute;
			//}
		...
		{
			var lst = ApplicationUserDataManager.GetList(SessionVariables.RequestProfile);
			var userName = "";
			for (int i = 0; i < lst.Count; i++)
			{
				if (lst[i].ApplicationUserId == SessionVariables.RequestProfile.AuditId)
				{
					userName = lst[i].ApplicationUserName;
					break;
				}
			}
			return userName;
                        ...
                        if (listObject != null)
                        {
                            ...
                            foreach (var item in listObject)
                            {
                                ...
                                if (elementProperty == null)
                                {
                                    ...
                                    {
										if (headProp.DeclaringType.Name != ApplicationCommon.BaseDataModel)
										{
                                ...
                                {
									if (prop.DeclaringType.Name != ApplicationCommon.BaseDataModel)
									{
