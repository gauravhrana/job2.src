...
{
	public class DailyTaskItemQueueStatusController : Controller
	{
		string settingCategory = "DailyTaskItemQueueStatusIndexView";
		...
		private string GetAllKendoColumns()
		{
			...
			str.AppendLine("            template: \"<input onclick='CheckWithHeader();' class='check-box' id='chkBox${DailyTaskItemQueueStatusId}' type='checkbox' value='${DailyTaskItemQueueStatusId}' /> \"");
			str.AppendLine("        },");
			...
			// Add Button Columns
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'DailyTaskItemQueueStatusId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueueStatus/Details/${DailyTaskItemQueueStatusId}'>Details</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'DailyTaskItemQueueStatusId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueueStatus/Edit/${DailyTaskItemQueueStatusId}'>Edit</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'DailyTaskItemQueueStatusId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueueStatus/Delete/${DailyTaskItemQueueStatusId}'>Delete</a>\"");
			str.AppendLine("        }");
			str.AppendLine("    ]");
		...
		private string GetKendoGridConfiguration(string searchString, int fcModeId)
		{
			...
			str.AppendLine("            read: \"/AreaSchedule/DailyTaskItemQueueStatus/IndexResult?searchString=" + searchString + "\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
			str.AppendLine("                        DailyTaskItemQueueStatusId: { type: \"number\" },");
		...
		public DailyTaskItemQueueStatusDataModel GetById(int id)
		{
			var obj = new DailyTaskItemQueueStatusDataModel();
			obj.DailyTaskItemQueueStatusId = id;
			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.GetEntityDetails(obj, SessionVariables.RequestProfile);
		...
		public JsonResult IndexResult(string name, int? page, int? pageSize)
		{
			var obj = new DailyTaskItemQueueStatusDataModel();
			obj.Name = name;
			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.GetEntitySearch(obj, SessionVariables.RequestProfile);
		...
		{
			var lstItems = new List<DailyTaskItemQueueStatusDataModel>();
			if (isMultiple != null && isMultiple.Value)
			{
				var lstEntityKeys = ApplicationCommon.GetSuperKeyDetails(SystemEntity.DailyTaskItemQueueStatus.Value(), id.ToString());
		...
		[HttpPost]
		public ActionResult Create(DailyTaskItemQueueStatusDataModel obj)
		{
			if (ModelState.IsValid)
			{
				TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Create(obj, SessionVariables.RequestProfile);
		...
		[HttpPost]
		public ActionResult Edit(List<DailyTaskItemQueueStatusDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Update(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
		...
		[HttpPost]
		public ActionResult Delete(List<DailyTaskItemQueueStatusDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Delete(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
