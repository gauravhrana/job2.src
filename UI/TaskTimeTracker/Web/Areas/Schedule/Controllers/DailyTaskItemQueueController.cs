...
namespace ApplicationContainer.UI.Web.Areas.Schedule.Controllers
{
	...
	public class DailyTaskItemQueueController : Controller
	{
		...
		private string GetAllKendoColumns()
		{
			...
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueue/Details/${DailyTaskItemQueueId}'>Details</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'DailyTaskItemQueueId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueue/Edit/${DailyTaskItemQueueId}'>Edit</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'DailyTaskItemQueueId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/AreaSchedule/DailyTaskItemQueue/Delete/${DailyTaskItemQueueId}'>Delete</a>\"");
			str.AppendLine("        }");
			str.AppendLine("    ]");
		...
		private string GetKendoGridConfiguration(string searchString, string dailyTaskItemQueueStatus, int fcModeId)
		{
			var str = new StringBuilder();
			...
			str.AppendLine("            read: \"/AreaSchedule/DailyTaskItemQueue/IndexResult?searchString=" + searchString + "&dailyTaskItemQueueStatus=" + dailyTaskItemQueueStatus +"\"");
			str.AppendLine("        },");
		...
		public DailyTaskItemQueueDataModel GetById(int id)
		{
			...
			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueDataManager.GetEntityDetails(obj, SessionVariables.RequestProfile);
		...
		public JsonResult IndexResult(string name, string dailyTaskItemQueueStatus, string businessDateMin, string businessDateMax, int? page, int? pageSize)
		{
			var obj = new DailyTaskItemQueueDataModel();
			obj.Name = name;
			
			if (!string.IsNullOrEmpty(dailyTaskItemQueueStatus) && dailyTaskItemQueueStatus != "-1")
			{
				obj.DailyTaskItemQueueStatusId = Convert.ToInt32(dailyTaskItemQueueStatus);
			}

			if (!string.IsNullOrEmpty(businessDateMin))
			{
				obj.BusinessDateMin = Shared.WebCommon.UI.Web.DateTimeHelper.FromUserDateFormatToDate(businessDateMin);
			}

			if (!string.IsNullOrEmpty(businessDateMax))
			{
				obj.BusinessDateMax = Shared.WebCommon.UI.Web.DateTimeHelper.FromUserDateFormatToDate(businessDateMax);
			}

			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueDataManager.GetEntitySearch(obj, SessionVariables.RequestProfile);
		...
		public JavaScriptResult IndexGridConfiguration(string searchString, int? page, int? pageSize)
		{
			...
			str.AppendLine("    dataSource: {");
			str.AppendLine("        transport: {");
			str.AppendLine("            read: \"/AreaSchedule/DailyTaskItemQueue/IndexResult\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
		...
		public ActionResult Index(string searchString, string dailyTaskItemQueueStatus, int? page, int? pageSize)
		{
			...
			ViewBag.KendoUIConfigurationString = GetKendoGridConfiguration(searchString, dailyTaskItemQueueStatus, firstFCMode);

			ViewBag.UserDateFormat = SessionVariables.UserDateFormat;
		...
		{	
			var data = new DailyTaskItemQueueStatusDataModel();
		
			var dtItems = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Search(data, SessionVariables.RequestProfile);
		...
		public ActionResult Create(DailyTaskItemQueueDataModel obj)
		{
			...
			{			
				TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueDataManager.Create(obj, SessionVariables.RequestProfile);
		...
		{
			var data = new DailyTaskItemQueueStatusDataModel();
			data.DailyTaskItemQueueStatusId = dailyTaskItemQueueStatusId;
		...
		public ActionResult Edit(List<DailyTaskItemQueueDataModel> items)
		{
			if (ModelState.IsValid)
			{
				...
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueDataManager.Update(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
		...
		public ActionResult Delete(List<DailyTaskItemQueueDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueDataManager.Delete(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
