...
namespace TaskTimeTracker.UI.Web.Areas.ReleaseNoteQualitative.Controllers
{
	public class DailyTaskItemQueueStatusController : Controller
	{
		...
		private string GetKendoGridConfiguration(string searchString, int fcModeId)
		{
			...
			str.AppendLine("            read: \"/ReleaseNoteQualitative/DailyTaskItemQueueStatus/IndexResult?searchString=" + searchString + "\"");
			str.AppendLine("        },");
		...
		public DailyTaskItemQueueStatusDataModel GetById(int id)
		{
			...
			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.GetEntityDetails(obj, SessionVariables.AuditId);
		...
		public JsonResult IndexResult(string name, int? page, int? pageSize)
		{
			var obj = new DailyTaskItemQueueStatusDataModel();
			obj.Name = name;
			var list = TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.GetEntitySearch(obj, SessionVariables.AuditId);
		...
		public ActionResult Create(DailyTaskItemQueueStatusDataModel obj)
		{
			...
			{
				TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Create(obj, SessionVariables.AuditId);
		...
		public ActionResult Edit(List<DailyTaskItemQueueStatusDataModel> items)
		{
			if (ModelState.IsValid)
			{
				...
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Update(item, SessionVariables.AuditId);
				}
				return RedirectToAction("Index");
			}
		...
		public ActionResult Delete(List<DailyTaskItemQueueStatusDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					TaskTimeTracker.Components.Module.TimeTracking.DailyTaskItemQueueStatusDataManager.Delete(item, SessionVariables.AuditId);
				}
				return RedirectToAction("Index");
			}
