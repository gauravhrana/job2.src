...
{
	public class ReleaseNoteTechnicalDifficultyController : Controller
	{

		string settingCategory = "ReleaseNoteTechnicalDifficultyIndexView";
		...
		private List<string> GetKendoColumnSetByFCMode(int fcModeId)
		{
			...
			var systemEntityTypeId = (int)SystemEntity.ReleaseNoteTechnicalDifficulty;
		...
		private string GetAllKendoColumns()
		{
			...
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", field: 'ReleaseNoteTechnicalDifficultyId',");
			str.AppendLine("            filterable: false, sortable: false, width:30,");
			str.AppendLine("            headerTemplate: \"<input type='checkbox' id='checkAll'/>\",");
			str.AppendLine("            template: \"<input onclick='CheckWithHeader();' class='check-box' id='chkBox${ReleaseNoteTechnicalDifficultyId}' type='checkbox' value='${ReleaseNoteTechnicalDifficultyId}' /> \"");
			str.AppendLine("        },");
			...
			// Add Button Columns
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseNoteTechnicalDifficultyId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseNoteTechnicalDifficulty/Details/${ReleaseNoteTechnicalDifficultyId}'>Details</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseNoteTechnicalDifficultyId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseNoteTechnicalDifficulty/Edit/${ReleaseNoteTechnicalDifficultyId}'>Edit</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseNoteTechnicalDifficultyId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseNoteTechnicalDifficulty/Delete/${ReleaseNoteTechnicalDifficultyId}'>Delete</a>\"");
			str.AppendLine("        }");
			str.AppendLine("    ]");
		...
		private string GetKendoGridConfiguration(string searchString, int fcModeId)
		{
			...
			str.AppendLine("            read: \"/ReleaseNoteQualitative/ReleaseNoteTechnicalDifficulty/IndexResult?searchString=" + searchString + "\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
			str.AppendLine("                        ReleaseNoteTechnicalDifficultyId: { type: \"number\" },");
		...
		public JavaScriptResult IndexGridConfiguration(string searchString, int? page, int? pageSize)
		{
			...
			str.AppendLine("    dataSource: {");
			str.AppendLine("        transport: {");
			str.AppendLine("            read: \"/ReleaseNoteQualitative/ReleaseNoteTechnicalDifficulty/IndexResult\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
		...
		public JsonResult GetSearchFilterColumns()
		{
			...
			data.SystemEntityTypeId = SystemEntity.ReleaseNoteTechnicalDifficulty.Value();
		...
		// GET: /ReleaseNoteTechnicalDifficulty/
		public ActionResult Index(string searchString, int? page, int? pageSize)
		{

			var dtFCModes = FieldConfigurationUtility.GetApplicableModesList(SystemEntity.ReleaseNoteTechnicalDifficulty);
		...
		//
		// GET: /ReleaseNoteTechnicalDifficulty/Details/5
		public ActionResult Details(int id, bool? isMultiple)
		{
			var lstItems = new List<ReleaseNoteTechnicalDifficultyDataModel>();
			if (isMultiple != null && isMultiple.Value)
			{
				var lstEntityKeys = ApplicationCommon.GetSuperKeyDetails(SystemEntity.ReleaseNoteTechnicalDifficulty.Value(), id.ToString());
		...
		//
		// GET: /ReleaseNoteTechnicalDifficulty/Create
		...
		//
		// POST: /ReleaseNoteTechnicalDifficulty/Create
		[HttpPost]
		...
		//
		// GET: /ReleaseNoteTechnicalDifficulty/Edit/5
		...
		//
		// POST: /ReleaseNoteTechnicalDifficulty/Edit

		[HttpPost]
		public ActionResult Edit(List<ReleaseNoteTechnicalDifficultyDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					ReleaseNoteTechnicalDifficultyDataManager.Update(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
		...
		//
		// GET: /ReleaseNoteTechnicalDifficulty/Delete/5
		...
		//
		// POST: /ReleaseNoteTechnicalDifficulty/Delete
		[HttpPost]
		public ActionResult Delete(List<ReleaseNoteTechnicalDifficultyDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					ReleaseNoteTechnicalDifficultyDataManager.Delete(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
