...
namespace Framework.UI.Web.BaseClasses
{
	...
	public abstract class ControlSearchFilter : BaseControl
	{
		...
		public delegate void LoadCheckedBoxListSources(string fieldName, CheckBoxList checkkBoxListControl);	

		public LoadComboBoxSources LoadComboBoxSourceMethod;
		public LoadCheckedBoxListSources LoadCheckedBoxListSourceMethod;
		...
		protected virtual void SetDefaultValues()
		{
			for (var i = 0; i < SearchParametersRepeaterCore.Items.Count; i++)
			{
				...
				var checkboxlist	= (CheckBoxList)SearchParametersRepeaterCore.Items[i].FindControl("checkboxlist");
				...
				else if (checkboxlist != null && checkboxlist.Visible)
				{
		...
		// this needs to be implemented in the inheriting class to load the checkboxlist in search control
		public virtual void LoadCheckBoxListSources(string fieldName, CheckBoxList checkBoxListControl)
		{
			if (LoadCheckedBoxListSourceMethod != null)
			{
				LoadCheckedBoxListSourceMethod(fieldName, checkBoxListControl);
			}
		}
		...
		public virtual object CheckAndGetFieldValue(string field, bool ddlReturnValue = true, bool retrieveValue = false)
		{
			for (var i = 0; i < SearchParametersRepeaterCore.Items.Count; i++)
			{
				...
				if (hdnfield.Value.Equals(field))
				{
					...
					var checkboxlist	= (CheckBoxList)item.FindControl("checkboxlist");
					...
					else if (checkboxlist != null && checkboxlist.Visible)
					{
						...
						for (int k = 0; k < checkboxlist.Items.Count; k++)
						{
							...
							{
								values += checkboxlist.Items[k].Value + ";";
							}							
						}
			...
			for (var i = 0; i < SearchParametersRepeaterCore.Items.Count; i++)
			{
				...
				if (hdnfield.Value.Equals(field))
				{
					...
					else if (checkboxlist != null && checkboxlist.Visible)
					{
						...
						{
							checkboxlist.SelectedIndex = checkboxlist.Items.IndexOf(checkboxlist.Items.FindByText(value));
							txtDevBox.Text = checkboxlist.SelectedValue;
						}
					}
			...
			foreach (DataRow dr in SearchColumns.Rows)
			{
				...
				switch (columnName)
				{
					...
					case "SubGroupBy":
						...
						if ((subGroupByValue != "None" || subGroupByValue != "-1" || subGroupByValue != "") && (groupByValue == "None" || groupByValue == "-1" || groupByValue == ""))
						{
		...
		public void SetupSearch()
		{
			...
			if (SearchColumns == null)
			{
				...
				if (drs.Length > 0)
				{
					...
					{
						PreferenceUtility.UpdateUserPreference(SettingCategory, "SubGroupBy", "");
					}
				}
                ...
                var enableGroupBy = PreferenceUtility.GetApplicationUserPreferenceByKey(ApplicationCommon.EnableGroupBy);

                if (enableGroupBy.Equals("True"))
                {
                    IsGroupByFeatureEnabled = true;
                }
		...
		protected virtual void SearchParametersRepeater_ItemDataBound(object sender, RepeaterItemEventArgs e)
		{
			...
			if (dt.Rows.Count >= 1)
			{
				...
				else if (controlType.Equals("CheckBoxList", StringComparison.OrdinalIgnoreCase))
				{
					ctrlInput = SetupCheckBoxList(plcControlHolder, txtDevBox, name);
				}
				else // shouldtype
				{
		...
		private CheckBoxList SetupCheckBoxList(PlaceHolder plcControlHolder, TextBox txtDevBox, string name)
		{
			...
			if (plcControlHolder != null)
			{
				...
				chkBoxList.ID = "checkboxlist";
				chkBoxList.EnableViewState = true;
				chkBoxList.AppendDataBoundItems = true;

				outerLink.Controls.Add(chkBoxList);
				//plcControlHolder.Controls.Add(chkBoxList);

				checkBoxList = chkBoxList;
			}

			txtDevBox.Visible = true;

			LoadCheckBoxListSources(name, checkBoxList);
