IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='TraderSearch') 
BEGIN
	DROP Procedure TraderSearch
END
GO

CREATE Procedure dbo.TraderSearch
(
		@TraderId				INT		= NULL
	,	@FundId				INT		= NULL
...
BEGIN
	...
	BEGIN
		...
		SET @InputParametersLocal		=  'TraderId' 
		SET @InputValuesLocal			=  CAST(@TraderId AS VARCHAR(50))

		EXEC dbo.StoredProcedureLogInsert
				@Name					= 'dbo.TraderSearch'
	...
	AND		a.TraderId =
			CASE
				WHEN @TraderId IS NULL THEN a.TraderId
				ELSE @TraderId
			END
	AND		a.FundId =
			CASE
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.TraderId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.TraderId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.TraderId=b.TraderId
		ORDER BY	a.TraderId
	END
	ELSE
	BEGIN
