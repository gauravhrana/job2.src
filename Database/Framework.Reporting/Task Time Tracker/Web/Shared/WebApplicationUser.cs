...
namespace Shared.UI.Web
{
    public class WebApplicationUser
    {
        ...
        public static bool CheckIfUserIsValid(int applicationUserId)
        {
			...
			var dt = Framework.Components.ApplicationUser.ApplicationUserDataManager.Search(data, SessionVariables.SystemRequestProfile);
            
			if (dt != null && dt.Rows.Count > 0)
            {
                isValid = true;
                SessionVariables.ApplicationUserName = Convert.ToString(dt.Rows[0][ApplicationUserDataModel.DataColumns.ApplicationUserName]);
            }
