...
namespace Shared.UI.Web
{
	public class DateRangeHelper
	{
		...
		public static string[] FillUpDate(string strDateRange,string dateformatRange)
		{
			...
			if (strDateRange == "All" || strDateRange == "Custom")
			{
			...
			else if (strDateRange == "Last Quarter")
			{
                ...
                var startingMonth = ((quarterNo - 1) * 3) + 1;

                var buffer = 3;
                var year = dtNow.Year;

                if (dtNow.Month <= buffer)
                {
                   var currentmonth = dtNow.Month;
                   startingMonth = 12;
                   startingMonth = startingMonth - buffer + currentmonth;
                   year = dtNow.Year - 1;
                }
				var firstDayLastQuarter = new DateTime(year, startingMonth, 1);
				var lastDayOfLastQuarter = firstDayLastQuarter.AddMonths(3).AddDays(-1);

				fromDate = firstDayLastQuarter.ToString(dateformatRange);
				toDate = lastDayOfLastQuarter.ToString(dateformatRange);
			}
