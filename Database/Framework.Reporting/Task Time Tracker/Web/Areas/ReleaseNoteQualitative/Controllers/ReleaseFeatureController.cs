...
{
    public class ReleaseFeatureController : Controller
    {
		string settingCategory = "ReleaseFeatureIndexView";
		...
		private List<string> GetKendoColumnSetByFCMode(int fcModeId)
		{
			...
			var systemEntityTypeId = (int)SystemEntity.ReleaseFeature;
		...
		private string GetAllKendoColumns()
		{
			...
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", field: 'ReleaseFeatureId',");
			str.AppendLine("            filterable: false, sortable: false, width:30,");
			str.AppendLine("            headerTemplate: \"<input type='checkbox' id='checkAll'/>\",");
			str.AppendLine("            template: \"<input onclick='CheckWithHeader();' class='check-box' id='chkBox${ReleaseFeatureId}' type='checkbox' value='${ReleaseFeatureId}' /> \"");
			str.AppendLine("        },");
			...
			// Add Button Columns
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseFeatureId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseFeature/Details/${ReleaseFeatureId}'>Details</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseFeatureId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseFeature/Edit/${ReleaseFeatureId}'>Edit</a>\"");
			str.AppendLine("        },");
			str.AppendLine("        {");
			str.AppendLine("            title: \" \", width: 140, field: 'ReleaseFeatureId',");
			str.AppendLine("            filterable: false,");
			str.AppendLine("            template: \"<a href='/ReleaseNoteQualitative/ReleaseFeature/Delete/${ReleaseFeatureId}'>Delete</a>\"");
			str.AppendLine("        }");
			str.AppendLine("    ]");
		...
		private string GetKendoGridConfiguration(string searchString, int fcModeId)
		{
			...
			str.AppendLine("            read: \"/ReleaseNoteQualitative/ReleaseFeature/IndexResult?searchString=" + searchString + "\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
			str.AppendLine("                        ReleaseFeatureId: { type: \"number\" },");
		...
		public ReleaseFeatureDataModel GetById(int id)
		{
			var obj = new ReleaseFeatureDataModel();
			obj.ReleaseFeatureId = id;
			var list = ReleaseFeatureDataManager.GetEntityDetails(obj, SessionVariables.RequestProfile);
		...
		public JsonResult IndexResult(string name, int? page, int? pageSize)
		{
			var obj = new ReleaseFeatureDataModel();
			obj.Name = name;
			var list = ReleaseFeatureDataManager.GetEntitySearch(obj, SessionVariables.RequestProfile);
		...
		public JavaScriptResult IndexGridConfiguration(string searchString, int? page, int? pageSize)
		{
			...
			str.AppendLine("    dataSource: {");
			str.AppendLine("        transport: {");
			str.AppendLine("            read: \"/ReleaseNoteQualitative/ReleaseFeature/IndexResult\"");
			str.AppendLine("        },");
			str.AppendLine("        schema: {");
			str.AppendLine("            model: {");
			str.AppendLine("                    fields: {");
		...
		public JsonResult GetSearchFilterColumns()
		{
			...
			data.SystemEntityTypeId = SystemEntity.ReleaseFeature.Value();
		...
		public ActionResult Index(string searchString, int? page, int? pageSize)
		{
			...
			var dtFCModes = FieldConfigurationUtility.GetApplicableModesList(SystemEntity.ReleaseFeature);
		...
		//
		// GET: /ReleaseFeature/Details/5
		public ActionResult Details(int id, bool? isMultiple)
		{
			var lstItems = new List<ReleaseFeatureDataModel>();
			if (isMultiple != null && isMultiple.Value)
			{
				var lstEntityKeys = ApplicationCommon.GetSuperKeyDetails(SystemEntity.ReleaseFeature.Value(), id.ToString());
		...
		//
		// POST: /ReleaseFeature/Create
		[HttpPost]
		public ActionResult Create(ReleaseFeatureDataModel obj)
		{
			if (ModelState.IsValid)
			{
				ReleaseFeatureDataManager.Create(obj, SessionVariables.RequestProfile);
		...
		//
		// POST: /ReleaseFeature/Edit

		[HttpPost]
		public ActionResult Edit(List<ReleaseFeatureDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					ReleaseFeatureDataManager.Update(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
		...
		//
		// POST: /ReleaseFeature/Delete
		[HttpPost]
		public ActionResult Delete(List<ReleaseFeatureDataModel> items)
		{
			if (ModelState.IsValid)
			{
				foreach (var item in items)
				{
					ReleaseFeatureDataManager.Delete(item, SessionVariables.RequestProfile);
				}
				return RedirectToAction("Index");
			}
