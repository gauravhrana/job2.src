IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='OrderTypeSearch') 
BEGIN
	DROP Procedure OrderTypeSearch
END
GO

CREATE Procedure dbo.OrderTypeSearch
(
...
BEGIN
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.OrderTypeId
		AND		c.SystemEntityId	= @SystemEntityTypeId
		AND		c.AuditActionId		IN (1,2)
		GROUP BY	c.EntityKey	

		-- Get Audit Date and CreatedByPersonId for given records
		SELECT	a.OrderTypeId
			,	c.AuditActionId 
			,	c.CreatedDate
			,	c.CreatedByPersonId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.OrderTypeId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.OrderTypeId=b.OrderTypeId
		ORDER BY	a.OrderTypeId
	END
	ELSE
	BEGIN
