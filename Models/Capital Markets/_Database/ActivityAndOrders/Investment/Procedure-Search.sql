IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='TxInvestmentSearch') 
BEGIN
	DROP Procedure TxInvestmentSearch
END
GO

CREATE Procedure dbo.TxInvestmentSearch
(
		@TxInvestmentId				INT		= NULL
...
BEGIN
	...
	AND		a.TxInvestmentId = ISNULL(@TxInvestmentId, a.TxInvestmentId)
	AND		a.TransactionEventId = ISNULL(@TransactionEventId, a.TransactionEventId)
	ORDER BY	a.TxInvestmentId ASC

	IF	@ApplicationMode = 1 
	BEGIN
		DELETE FROM #TempMain
		WHERE TxInvestmentId < 0
	END
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.TxInvestmentId
		AND		c.SystemEntityId	= @SystemEntityTypeId
		AND		c.AuditActionId		IN (1,2)
		GROUP BY	c.EntityKey	

		-- Get Audit Date and CreatedByPersonId for given records
		SELECT	a.TxInvestmentId
			,	c.AuditActionId 
			,	c.CreatedDate
			,	c.CreatedByPersonId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.TxInvestmentId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.TxInvestmentId=b.TxInvestmentId
		ORDER BY	a.TxInvestmentId
	END
	ELSE
	BEGIN
