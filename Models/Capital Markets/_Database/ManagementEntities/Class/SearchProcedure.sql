IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='ClassSearch') 
BEGIN
	DROP Procedure ClassSearch
END
GO

CREATE Procedure dbo.ClassSearch
(
		@ClassId				INT		= NULL
	,	@FundId				INT		= NULL
...
BEGIN
	...
	BEGIN
		...
		SET @InputParametersLocal		=  'ClassId' 
		SET @InputValuesLocal			=  CAST(@ClassId AS VARCHAR(50))

		EXEC dbo.StoredProcedureLogInsert
				@Name					= 'dbo.ClassSearch'
	...
	AND		a.ClassId =
			CASE
				WHEN @ClassId IS NULL THEN a.ClassId
				ELSE @ClassId
			END
	AND		a.FundId =
			CASE
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.ClassId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.ClassId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.ClassId=b.ClassId
		ORDER BY	a.ClassId
	END
	ELSE
	BEGIN
