...
BEGIN
	...
	BEGIN
		...
		SET @InputParametersLocal		=  'SeriesId' 
		SET @InputValuesLocal			=  CAST(@SeriesId AS VARCHAR(50))
	...
	AND		a.SeriesId =
			CASE
				WHEN @SeriesId IS NULL THEN a.SeriesId
				ELSE @SeriesId
			END
	AND		a.FundId =
			CASE
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.SeriesId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.SeriesId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.SeriesId=b.SeriesId
		ORDER BY	a.SeriesId
	END
	ELSE
	BEGIN
