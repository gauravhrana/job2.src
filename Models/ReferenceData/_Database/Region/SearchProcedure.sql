...
BEGIN
	...
	BEGIN
		...
		SET @InputParametersLocal		=  'RegionId' 
		SET @InputValuesLocal			=  CAST(@RegionId AS VARCHAR(50))
	...
	AND		a.RegionId =
			CASE
				WHEN @RegionId IS NULL THEN a.RegionId
				ELSE @RegionId
			END
	AND		a.CountryId =
			CASE
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.RegionId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.RegionId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.RegionId=b.RegionId
		ORDER BY	a.RegionId
	END
	ELSE
	BEGIN
