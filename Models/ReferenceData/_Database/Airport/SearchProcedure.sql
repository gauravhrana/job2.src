IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='AirportSearch') 
BEGIN
	DROP Procedure AirportSearch
END
GO

CREATE Procedure dbo.AirportSearch
(
		@AirportId				INT		= NULL
	,	@CountryId				INT		= NULL
...
BEGIN
	...
	BEGIN
		...
		SET @InputParametersLocal		=  'AirportId' 
		SET @InputValuesLocal			=  CAST(@AirportId AS VARCHAR(50))

		EXEC dbo.StoredProcedureLogInsert
				@Name					= 'dbo.AirportSearch'
	...
	AND		a.AirportId =
			CASE
				WHEN @AirportId IS NULL THEN a.AirportId
				ELSE @AirportId
			END
	AND		a.CountryId =
			CASE
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.AirportId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.AirportId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.AirportId=b.AirportId
