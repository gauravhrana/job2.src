IF  EXISTS (SELECT * FROM sysobjects WHERE type = 'P' AND name ='CaseTypeSearch') 
BEGIN
	DROP Procedure CaseTypeSearch
END
GO

CREATE Procedure dbo.CaseTypeSearch
(
		@CaseTypeId				INT		= NULL
...
BEGIN
	...
	BEGIN
		...
		INTO		#HistortyInfo
		FROM 		#TempMain a	
		INNER JOIN	CommonServices.dbo.AuditHistory c ON	c.EntityKey			= a.CaseTypeId
		...
		FROM		#TempMain a
		INNER JOIN	#HistortyInfo							b ON	b.EntityKey			= a.CaseTypeId
		...
		SELECT	a.*
			...
			,	b.LastAction
		FROM	#TempMain		a
		LEFT JOIN	#HistortyInfoDetails	b ON	a.CaseTypeId=b.CaseTypeId
		ORDER BY	a.CaseTypeId
	END
	ELSE
	BEGIN
